...
uint32_t c2c_fft(const float *input, float *output, uint32_t size)
{
	...
	handle_error(cudaMemcpy(device_input, input, sizeof(float2) * size, cudaMemcpyHostToDevice));
	...
	handle_error(cudaMemcpy(output, device_output, sizeof(float2) * size, cudaMemcpyDeviceToHost));
	for (i = 0; i < size; i++)
	{
